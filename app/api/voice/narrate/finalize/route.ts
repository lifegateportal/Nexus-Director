  ...
  try {
    ...
    if (!output) {
      return NextResponse.json({
        status: "FAILED",
        error: `RunPod synthesis completed without structured output (${summarizePayloadShape(json)})`,
      });
    }
    ...
    if (!wavB64) {
      return NextResponse.json({
        status: "FAILED",
        error: `RunPod synthesis completed without audio base64 payload (${summarizePayloadShape(output)})`,
      });
    }
