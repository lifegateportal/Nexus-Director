...
export async function POST(req: NextRequest) {
  ...
  // Capped at ~2500 words total to keep DeepSeek latency well under maxDuration.
  const words = input.masterTranscript.split(/\s+/);
  const total = words.length;
  const startSample = words.slice(0, 900).join(" ");
  const midStart = Math.max(900, Math.floor(total / 2) - 400);
  const midSample = words.slice(midStart, midStart + 800).join(" ");
  const endSample = words.slice(Math.max(0, total - 800)).join(" ");
